...
pair<ll,ll> rec(ll bit, ll v, ll tim) {
  //if (dp.at(bit).at(v).first != -1 && dp.at(bit).at(v).first < INF) {
  //  return dp.at(bit).at(v);
  //}
  
  if (bit==(1<<v)) {
    if (dist.at(0).at(v) != -1 && tim + dist.at(0).at(v) <= times.at(0).at(v)) {
      return make_pair(dist.at(0).at(v), 1);
    } else {
  ...
  rep(i,0,V) {
    if ((bit2 & (1<<i)) && dist.at(i).at(v) != -1) {
      ...
      pair<ll,ll> tmp_p = rec(bit2, i, tim+dist.at(i).at(v));
...
int main() {
  ...
  pair<ll,ll> res_p = rec((1<<V)-1, 0, 0);;
