...
int main() {
  ...
  vector<vector<int>> vec(N, vector<int>(100));
  
//  vec.at(0).at(0) = 1;
  rep(i,0,N) vec.at(i).at(0) = 1;
  rep(i,0,N) vec.at(i).at(i) = 1;
  
  rep(i,1,N) {
    rep(j,1,i+1) {
      vec.at(i).at(j) = vec.at(i-1).at(j-1) + vec.at(i-1).at(j);
    }
  }
  
  rep(i,0,N) {
    rep(j,0,i+1) {
      if (j!=i) cout << vec.at(i).at(j) << " ";
